...
<div id="modal-comment-edit" data-dashboard-modal class="fixed inset-0 z-[100] hidden" aria-hidden="true">
    ...
    <div class="relative flex min-h-full items-end justify-center p-4 sm:items-center sm:p-6">
        <div role="dialog" aria-modal="true" aria-labelledby="modal-comment-edit-title" tabindex="-1"
            ...
            <form class="space-y-4 px-5 py-4" method="post" action="<?= htmlspecialchars($commentsUrl, ENT_QUOTES, 'UTF-8') ?>">
                <input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8') ?>"/>
                <input type="hidden" name="_action" value="update"/>
                <input type="hidden" name="_redirect_query" value="<?= htmlspecialchars($redirectQuery, ENT_QUOTES, 'UTF-8') ?>"/>
                <input type="hidden" name="id" id="comment-edit-id" value=""/>
                <div>
                    <p class="<?= $lbl ?>">Post</p>
                    <p class="mt-1.5 rounded-xl border border-stone-100 bg-stone-50/80 px-3 py-2 text-sm text-stone-800" data-readonly-post-title>—</p>
                </div>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="<?= $lbl ?>" for="comment-edit-author">Author</label>
                        <input id="comment-edit-author" name="author_name" type="text" required maxlength="191" class="<?= $fld ?>"/>
                    </div>
                    <div>
                        <label class="<?= $lbl ?>" for="comment-edit-email">Email</label>
                        <input id="comment-edit-email" name="author_email" type="email" required maxlength="191" class="<?= $fld ?>"/>
                    </div>
                </div>
                <div>
                    <label class="<?= $lbl ?>" for="comment-edit-state">Status</label>
                    <select id="comment-edit-state" name="status" class="<?= $dashboardSelect ?> mt-1.5">
                ...
                <div>
                    ...
                    <textarea id="comment-edit-body" name="body" rows="5" required maxlength="8000" class="<?= $fld ?>"></textarea>
                </div>
                <div class="flex flex-col-reverse gap-2 border-t border-stone-100 pt-4 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="<?= $btnPri ?> w-full sm:w-auto">Save comment</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="modal-comment-view-title" tabindex="-1"
            ...
            <div class="space-y-4 px-5 py-4 text-sm">
                ...
                <div>
                    ...
                    <p class="mt-2 whitespace-pre-wrap leading-relaxed text-stone-700" data-view="comment-body">—</p>
                </div>
            </div>
